...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        cout << " R1: Total keypoints on preceding vehicle : " << keypoints.size() <<endl;

        if (keypoints.size() > 0)
        {
            auto in = keypoints[0];
            cout << " R1: Neighbhorhood size NSNSNSNSNSNSNS     : " << in.size <<endl;
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            cout << " R2: Matching keypoints MKMKMKMKMKMKMKMKMK         : " << matches.size() << endl;
